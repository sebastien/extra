from typing import Optional, Iterable, Callable, Union, Any, Generic, TypeVar, IO, cast
...
T = TypeVar("T")
__doc__ = """ Defines different types of nodes that can be used to work with
tree-like structures. Each tree structure supports a few useful features:
...
class Event:
    ...
    def __init__(self, name: str, data: Optional[Any] = None):
        ...
        self.target: Optional["BaseNode"] = None
...
class BaseNode(Generic[N]):
    ID = 0
    SEPARATOR = "."

    def __init__(self):
        self.id: int = BaseNode.ID
        BaseNode.ID += 1
        self.parent: Optional[N] = None
    ...
    @property
    def descendants(self) -> Iterable[N]:
        for child in self.children:
            yield child
            yield from child.descendants

    @property
    def leaves(self) -> Iterable[N]:
    ...
    def setMeta(self, meta: dict[str, str]) -> N:
        self.meta = meta
        return cast(N, self)
    ...
    def walk(self) -> Iterable[N]:
        yield cast(N, self)
        for c in self.children:
            yield from c.walk()

    def on(self, event: str, callback: Callable) -> N:
    ...
    def off(self, event: str, callback: Callable) -> N:
    ...
    def trigger(self, event: str, data=None) -> Event:
        ...
        up."""
        return self._dispatchEvent(Event(event, data))
    ...
    def clear(self) -> list[N]:
        return [child.detach() for child in self.children]

    def remove(self, node: N) -> N:
        raise NotImplemented

    def detach(self) -> N:
        return self.parent.remove(self) if self.parent else cast(N, self)
    ...
    def __str__(self) -> str:
        return f"({self.__class__.__name__}:{self.name} +{len([_ for _ in self.children])})"
...
class NamedNode(BaseNode["NamedNode"]):
    ...
    def __init__(
        ...
        self._children: Union[
            weakref.WeakValueDictionary[str, "NamedNode"], dict[str, "NamedNode"]
        ] = {}
...
#
#     @staticmethod
#     def DATA_EXTRACT(_: str) -> str:
#         return _[: -(len(".data.json.gz"))]
#
#     @staticmethod
#     def META_EXTRACT(_: str) -> str:
...
#
#     @staticmethod
#     def SERIALIZE(data: Any, stream: IO[str]):
...
class ASCII:
    ...
    def Format(cls, node: Node, prefix="") -> Iterable[str]:
        ...
        last_child = len([_ for _ in node.children]) - 1
        prefix = " " * len(prefix)
